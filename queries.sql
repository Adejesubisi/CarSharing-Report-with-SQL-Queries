...
-- Creating Table for Weekday, Season and Month
...
-- - **(e)**: Weather analysis of month with highest demand.
--Month with Highest Average Demand Temperature Category
CREATE TABLE MonHadTempC( 
Temp_Category	TEXT,
avg_demand	REAL
);
INSERT INTO MonHadTempC
SELECT tem.temp_category, AVG(car.demand) AS avg_demand
FROM Temperaturetab tem
JOIN CarSharing_df car ON tem.temp_code = car.temp_code
JOIN Timetab tim on car.id = tim.id
...
SELECT * FROM MonHadTempC

-- Month with Highest Average Demand weather frequency
CREATE TABLE MonHadWeathF (
Weather	TEXT,
Frequency
);
INSERT INTO MonHadWeathF
SELECT weather, COUNT(*) AS Frequency
FROM weathertab wea
JOIN CarSharing_df car ON wea.weather_code = car.weather_code
JOIN Timetab tim on car.id = tim.id
